 ...
 * 这个类是openTSDB的中心实现类。这个类的功能有：往数据库中插入数据、查询数据。
 */
  ...
  public void initializePlugins(final boolean init_rpcs) {
    ...
    if (config.getBoolean("tsd.uidfilter.enable")) {
      uid_filter = PluginLoader.loadSpecificPlugin(
          config.getString("tsd.uidfilter.plugin"),UniqueIdFilterPlugin.class);
      if (uid_filter == null) {//紧接着对上面的uid_filter判断是否为null
        throw new IllegalArgumentException(
