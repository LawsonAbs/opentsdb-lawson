...
final class CliUtils {
  ...
  /**
   * Invokes the reflected {@code UniqueId.fromBytes()} method with the given
   * byte array using the UniqueId character set.
   *调用反射方法：UniqueId.fromBytes()，其参数是给出的字节数组，使用UniqueId 字符集。
   *
   * @param b The byte array to convert to a string   --> 需要转换成字符串的字节数组
   * @return The string
   * @throws RuntimeException if reflection failed
   */
