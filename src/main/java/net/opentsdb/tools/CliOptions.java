...
final class CliOptions {
   ...
   * 尝试加载已知的配置文件，否则是默认的配置。并用命令行参数覆盖
   *
   * 逻辑如下：
   * 1.读取命令行参数（其实也是配置信息）中是否有文件名。如果有的话，则将这个作为配置信息。
   * 否则程序将读取默认的配置文件config = new Config(true)
   * 2.同时，将命令行的参数覆盖配置文件中的参数
   * 3.
   * 4.
  ...
  static final Config getConfig(final ArgP argp) throws IOException {
    final Config config;// load configuration

    final String config_file = argp.get("--config", "");//得到--config对应的配置文件名
    if (!config_file.isEmpty())//如果config_file不为空，则进入if中
      config = new Config(config_file);//则为config指向新new出来的Config
    else
      config = new Config(true);//否则指向一个默认的配置
      //正是在构造函数中，才设置了本程序使用的默认配置文件
    ...
    // 配置——metric自动创建被记录到类中的一个flag中，因为它经常被用到。在复写之后，我们不得不手动设置。
